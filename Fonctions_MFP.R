...
BootTest <- function(pval=pval,data,NbBoot,formula,coup=coup,codage=X1ter,Z,txcensure,var.cod,temps,evt,ind.ajust,NamesY1,varcod){
  ...
  for(nbBoot in 1:NbBoot){
    dataBoot <- generate(n,alphahat,etahat,c(betahat,0),Z=Z,X=var.cod,censor=txcensure,temps,evt,varcod)
...
resampling <- function (formula, data, varcod, FP, N = 1000,txcensure,method,alpha){
  ...
  Z <- as.matrix(Z)
  t.obs <- LRTest(Z=Z,coup=coup,ind.ajust=ind.ajust,codage=X1ter,data=data,NamesY1=NamesY1)
  ...
  bestPF1 <- FP[bestPF,1:ncol(FP)]
  
  #print(bestPF1)
  
  
  if(method=="Bootstrap"){
    pval.Bootstrap <- BootTest(pval=pval,data=data,NbBoot=N,formula=formula1,coup=coup,codage=X1ter,Z=Z,txcensure=txcensure,var.cod=var.cod,temps=temps,evt=evt,ind.ajust=ind.ajust,NamesY1=NamesY1,varcod=varcod)
    pval_fb <- ifelse(pval.Bootstrap[[1]]<0.0001,"<0.0001",pval.Bootstrap[[1]])
    a <- list(Conclusion=paste("Based on bootstrap method,  the fractionnal polynomial number",bestPF, ", is selected with a p.value=",pval_fb),
           BestPF=bestPF1,raw.pval=pval)
    a
  }
  
  if(method=="Permutation"){
    pval.Permut <- PermutTest(pval=pval,data,Nbpermut=N,X1ter,var.cod=var.cod,coup=coup,ind.ajust,NamesY1,Z)
    pval_fp <- ifelse(pval.Permut[[1]]<0.0001,"<0.0001",pval.Permut[[1]])
    a <- list(Conclusion=paste("Based on permutation method,  the fractionnal polynomial number",bestPF, ",is selected with a p.value=",pval_fp),
                BestPF=bestPF1,raw.pval=pval)
    a
  } 
